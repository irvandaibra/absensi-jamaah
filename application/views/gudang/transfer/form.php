...
<body>
    <div class="all">
        ...
        <div id="main-wrapper">
            ...
            <div class="mt-5 pt-5 pb-5 px-5">
                <?php echo form_open_multipart('transfer/tambah') ?>
                <div class="page-wrapper shadow-lg p-3 mb-5 rounded" style="height: auto; background-color: white;">
                    <div class="page-breadcrumb">
                        <div class="row">
                            <div class="col-6 align-self-center">
                                <h2 class="page-title">Tambah Transfer</h2>
                            </div>
                        </div>
                    </div>
                    <div class="container-fluid">
                        <div class="mb-n3">
                            <h5>Form Tambah Transfer</h5>
                        </div>
                        <hr>
                        <div class="row mt-4 px-2">
                            ...
                            <div class="col-md-12 mt-n2 mb-3">
                                <label for="asal" class="form-label">Asal Transfer</label>
                                <select name="id_dari_toko" id="id_dari_toko" class="form-select select2 border-danger">
                                    <option value="">Pilih Toko</option>
                                    <?php
                                        foreach($toko as $rows) {
                                            echo '<option value="' . $rows->id . '">' . $rows->nama_toko . '</option>';
                                        }
                                    ?>
                                </select>
                            </div>
                            <div class="col-md-12 mt-n2 mb-3">
                                ...
                                <select name="id_ke_toko" id="id_ke_toko" class="form-select select2 border-danger">
                                    <option value="">Pilih Toko</option>
                                    <?php
                                        foreach($toko as $rows) {
                                            echo '<option value="' . $rows->id . '">' . $rows->nama_toko . '</option>';
    ...
    $(document).ready(function() {
        $('#id_dari_toko').change(function() {
            var id_toko = $('#id_dari_toko').val();
            $.ajax({
                url: '<?= base_url(); ?>transfer/get_produk_bytoko_id',
                method: 'POST',
                data: {
                    id_toko: id_toko
                },
    ...
    function pilihBarang(id) {
        if (id !== null) {
            $.ajax({
                url: '<?= base_url(); ?>transfer/get_produk_byid/' + id,
                method: 'POST',
                data: {
                    id: id
                },
