                        ...
                        <table class="table table-hover table-secondary w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Total Diharapkan</th>
    ...
    <script>
        $(document).ready(function() {
            $('.table').DataTable({
                ...
                ajax:
                {
                    url: "<?= base_url('shift/get_data/') ?>" + $('#toko').val(),
                    type: "POST"
                },
                deferRender: true,
                ...
                columnDefs: [
                    ...
                    },
                    {
                        targets: 5,
                        render: function (data, type, row, meta) {
                            return formatRupiah(data - row.total_didapat);
                        }
                    },
                    {
                        targets: 6,
                        render: function (data, type, row, meta) {
                            return `<a href="<?php echo base_url('shift/detail/')?>${data}" class="btn btn-sm btn-primary w-100">Detail</a>`;
                        }
                    }
                ],
            });
            $("#toko").on('change', function() {
                ...
                $('.table').DataTable({
                    ...
                    ajax:
                    {
                        url: "<?= base_url('shift/get_data/') ?>" + id,
                        type: "POST"
                    },
                    deferRender: true,
                    destroy: true,
        ...
        const impl = document.getElementById('export_shift');

        xhr.onload = function() {
            if (this.status === 200) {
                var data = JSON.parse(xhr.responseText);
                    impl.innerHTML += `<a class="dropdown-item" href="<?php echo base_urL("shift/export/") ?>${data[0].id}">Export Shift</a>`;
            } else {
                console.warn('ERROR CAUGHT!: UNKNOWN');
            }
        }

        xhr.open('get', '<?= base_url('shift/get_data_toko/') ?>' + $('#toko').val());
        xhr.send();
        ...
        $(document).ready(function(){
            $('#toko').change(function(){ 
                var id = $(this).val();
                $.ajax({
                    url : "<?= base_url('shift/get_data_toko/');?>" + id,
                    method : "POST",
                    data : {id_toko: id},
                    async : true,
                    dataType : 'json',
                    ...
                    success: function(data){
                        var impl = '';
                            impl += `<a class="dropdown-item" href="<?php echo base_urL("shift/export/") ?>${data[0].id}">Export Shift</a>`;
                        $('#export_shift').html(impl);
                    }
                });
                return false;
            }); 
