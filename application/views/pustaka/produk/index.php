                ...
                <div class="page-breadcrumb">
                    <div class="row">
                        ...
                        <div class="col-6 row justify-content-end">
                            <div class="col-4 mx-n1 d-flex justify-content-end">
                                <button type="button" class="btn btn-outline-secondary">Total :
                                    <?php echo $totaldata?></button>
                            </div>
                            <div class="col-4 mx-n1">
                            ...
                            <div class="col-4 mx-n1">
                                <a href="<?php echo base_url('produk/tambah')?>" class="btn btn-primary w-100">
                                    Tambah Produk
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="scroll-table">
                        <table class="table table-hover table-secondary w-100">
                            <thead>
                                <tr>
                                    <th>Nama</th>
                                    <th class="text-center" class="text-center">Kategori</th>
                                    <th class="text-center">Penetapan Harga</th>
    ...
    <script>
        $(document).ready(function() {
            $('.table').DataTable({
                ...
                ajax: {
                    url: "<?= base_url('produk/get_data/') ?>" + $('#toko').val(),
                    type: "POST"
                },
                deferRender: true,
                aLengthMenu: [
                ...
                columnDefs: [
                    ...
                    },
                    {
                        targets: 5,
                        render: function(data, type, row, meta) {
                            return `<a href="<?php echo base_url('produk/ubah/')?>${data}" class="btn btn-sm btn-primary">Ubah</a>`;
                        }
                    },
                ],
            });
            $("#toko").on('change', function() {
                ...
                $('.table').DataTable({
                    ...
                    ajax: {
                        url: "<?= base_url('produk/get_data/') ?>" + id,
                        type: "POST"
                    },
                    deferRender: true,
                    destroy: true,
        ...
        xhr.onload = function() {
            if (this.status === 200) {
                var data = JSON.parse(xhr.responseText);
                        impl.innerHTML += `<a class="dropdown-item" href="<?php echo base_urL("produk/export/") ?>`+$('#toko').val()+`">Export Items</a>`;
                    console.log(data);
            } else {
        ...
        xhr.open('get', '<?= base_url('produk/get_data_toko/') ?>' + $('#toko').val());
        xhr.send();
        ...
        $(document).ready(function(){
            $('#toko').change(function(){ 
                var id = $(this).val();
                $.ajax({
                    url : "<?php echo base_url('produk/get_data_toko/');?>" + id,
                    method : "POST",
                    data : {id_toko: id},
                    async : true,
                    dataType : 'json',
                    ...
                    success: function(data){
                        console.log(data);
                        var impl = '';
                            impl += `<a class="dropdown-item" href="<?php echo base_urL("produk/export/") ?>`+$('#toko').val()+`">Export Items</a>`;
                        $('#export').html(impl);
                    }
                });
                return false;
