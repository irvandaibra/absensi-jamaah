...
<body>
    <div class="all">
        ...
        <div id="main-wrapper">
            ...
            <?php echo form_open_multipart('tipe_penjualan/'.$this->uri->segment(2).$this->uri->segment(2) == 'ubah' && '/'.$this->uri->segment(3), 'class="mt-5 pt-5 pb-5 px-5"') ?>
            <div class="page-wrapper shadow-lg p-3 mb-5 rounded" style="height: 330px; background-color: white;">
                <div class="page-breadcrumb">
                    <div class="row">
                        <div class="col-6 align-self-center">
                            <h2 class="page-title"><?php echo $page?> Tipe Penjualan</h2>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-5 mt-4">
                            <?php
                                ...
                                echo form_dropdown($id_toko, $options, $id_toko['value'], 'class="form-select w-50 select2"');
                            ?>
                        </div>
                    </div>
                </div>
                <div class="container-fluid">
                    <div class="mb-n3">
                        <h5>Form <?php echo $page?> Tipe Penjualan</h5>
                    </div>
                    <hr>
                    <div class="mt-4 px-2">
                        <div class="mt-n2 mb-3">
                            <label for="nama" class="form-label">Nama Tipe Penjualan</label>
                            <?php echo form_input($nama_tipe, '', 'class="form-control" id="nama" autocomplete="off" placeholder="Nama Tipe Penjualan" required') ?>
                        </div>
                        <?php if ($page === 'Ubah') { ?>
                        <button type="button"
                            onClick="location.href='<?php echo base_url('tipe_penjualan/hapus/'.$row['id'])?>'"
                            class="btn btn-danger">
                            Hapus
                        </button>
                        <?php if ($row['status'] === 'Aktif') {?>
                        <button type="button"
                            onClick="location.href='<?php echo base_url('tipe_penjualan/nonaktifkan/'.$row['id'])?>'"
                            class="btn btn-danger">
                            Nonaktifkan
                        </button>
                        <?php } else if ($row['status'] === 'Nonaktif') {?>
                        <button type="button"
                            onClick="location.href='<?php echo base_url('tipe_penjualan/aktifkan/'.$row['id'])?>'"
