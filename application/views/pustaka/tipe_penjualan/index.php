                ...
                <div class="page-breadcrumb">
                    <div class="row">
                        <div class="col-6 align-self-center">
                            <h2 class="page-title">Tipe Penjualan</h2>
                        </div>
                        <div class="col-6 row justify-content-end">
                            <div class="col-4 mx-n1">
                                <a href="<?php echo base_url('tipe_penjualan/tambah')?>" class="btn btn-primary">Tambah
                                    Tipe Penjualan</a>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('.table').DataTable({
                ...
                order: [[0, 'asc']],
                ajax: {
                    url: "<?= base_url('tipe_penjualan/get_data/') ?>" + $('#toko').val(),
                    type: "POST"
                },
                deferRender: true,
                aLengthMenu: [
                ...
                columnDefs: [
                    {
                        ...
                        render: function (data, type, row, meta) {
                            if (data === 'Aktif') {
                            return `<span class="text-center text-success">Aktif</span>`
                            } else {
                            return `<span class="text-center text-danger">Nonaktif</span>`
                            };
                        }
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            return `<a href="<?php echo base_url('tipe_penjualan/ubah/')?>${data}" class="btn btn-sm btn-primary">Ubah</a>`;
                        }
                    },
                ]
            });
            $("#toko").on('change', function() {
                ...
                $('.table').DataTable({
                    ...
                    order: [[0, 'asc']],
                    ajax: {
                        url: "<?= base_url('tipe_penjualan/get_data/') ?>" + id,
                        type: "POST"
                    },
                    deferRender: true,
                    destroy: true,
